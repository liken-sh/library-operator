...
| <span id="status--phase"></span>`phase` | string | no | What the scanner is doing. Scanning while it walks the volume, Idle between walks, Pending while the storage or the catalog is not ready, and Offline when no scanner is running. Departing means the Library is deleted and the operator holds it open while a cleanup pod takes this library's rows out of the namespace's catalog; the Departing condition says which step that teardown has reached. |
