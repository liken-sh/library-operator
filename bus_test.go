...
func startBus(t *testing.T, count int, will *busWill, handler busHandler) (*Bus, []*fakeBroker, <-chan *Bus) {
	...
	bus := newBus("pipe", "library-operator", will, func(b *Bus) { connected <- b }, handler)
...
// it and then closes. It is every way a handshake fails short of a
// dead socket.
func misbehavingBroker(conn net.Conn, answer []byte) {
	go func() {
		defer conn.Close()
		if _, _, err := readPacket(bufio.NewReader(conn)); err != nil {
			return
		}
		conn.Write(answer)
	}()
...
// A handshake that does not reach a CONNACK ends the session, and the
// client dials again. The second connection completing is what proves
// the first ended.
func TestBusDialsAgainWhenTheHandshakeFails(t *testing.T) {
	cases := []struct {
		name   string
		answer []byte
	}{
		{name: "the broker refuses the connection", answer: []byte{mqttConnack, 0x02, 0x00, 0x05}},
		{name: "the broker answers with a packet that is not a CONNACK", answer: []byte{mqttPingresp, 0x00}},
		{name: "the broker closes the connection", answer: nil},
	}
