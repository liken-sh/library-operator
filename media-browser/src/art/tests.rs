...
use super::store::{Scaled, Store};
use super::{Art, Fit, Image};
...
impl Volume {
    ...
    fn store(&self, budget: usize, waker: Waker) -> Store {
        self.pool(budget, waker, 1)
    }

    fn pool(&self, budget: usize, waker: Waker, workers: usize) -> Store {
        let roots = HashMap::from([("movies".to_owned(), self.root.clone())]);
        Store::with_workers(roots, budget, waker, workers)
    }
...
fn pixel_at(scaled: &Scaled, x: u32, y: u32) -> [u8; 4] {
    let at = ((y * scaled.width + x) * 4) as usize;
    scaled.rgba[at..at + 4].try_into().expect("four bytes")
}

fn assert_solid(scaled: &Scaled, pixel: [u8; 4]) {
    assert_eq!(
        scaled.rgba.len(),
        (scaled.width * scaled.height * 4) as usize
    );
    let off = scaled
        .rgba
        .chunks_exact(4)
...
fn a_miss_decodes_in_the_background_and_becomes_a_hit() {
    ...
    assert!(
        store
            .scaled("movies", "poster.jpg", 32, 32, Fit::Cover)
            .is_none()
    );
    ...
    assert_eq!(scaled.rgba.len(), 32 * 32 * 4);
}
...
fn a_wide_source_is_scaled_to_cover_and_cropped_to_its_center() {
    ...
    assert!(
        store
            .scaled("movies", "wide.png", 20, 20, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    let scaled = store
...
fn a_tall_source_is_scaled_to_cover_and_cropped_to_its_center() {
    ...
    assert!(
        store
            .scaled("movies", "tall.png", 20, 20, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    let scaled = store
...
#[test]
fn the_least_recently_used_art_leaves_when_the_budget_fills() {
    ...
    assert!(store.scaled("movies", "a.png", 8, 8, Fit::Cover).is_none());
    wait_for_wakes(&wakes, 1);
    assert!(store.scaled("movies", "b.png", 8, 8, Fit::Cover).is_none());
    wait_for_wakes(&wakes, 2);
    assert!(store.scaled("movies", "b.png", 8, 8, Fit::Cover).is_some());
    assert!(store.scaled("movies", "a.png", 8, 8, Fit::Cover).is_some());
    assert!(store.scaled("movies", "c.png", 8, 8, Fit::Cover).is_none());
    wait_for_wakes(&wakes, 3);
    assert!(store.scaled("movies", "a.png", 8, 8, Fit::Cover).is_some());
    assert!(store.scaled("movies", "c.png", 8, 8, Fit::Cover).is_some());
    assert!(store.scaled("movies", "b.png", 8, 8, Fit::Cover).is_none());
}
...
fn a_missing_file_is_cached_as_a_failure_and_never_retried() {
    ...
    assert!(
        store
            .scaled("movies", "absent.jpg", 16, 16, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    assert!(
...
#[test]
fn a_failed_decode_marks_a_delivery_the_way_art_does() {
...
fn a_corrupt_file_is_cached_as_a_failure_and_never_retried() {
    ...
    assert!(
        store
            .scaled("movies", "bad.jpg", 16, 16, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    assert!(
...
fn repeated_requests_for_one_key_decode_once() {
    ...
    assert!(
        store
            .scaled("movies", "big.png", 300, 450, Fit::Cover)
            .is_none()
    );
    store.scaled("movies", "big.png", 300, 450, Fit::Cover);
    store.scaled("movies", "big.png", 300, 450, Fit::Cover);
    store.scaled("movies", "big.png", 300, 450, Fit::Cover);
    wait_for_wakes(&wakes, 1);
    assert!(
...
fn the_default_pool_decodes_too() {
    ...
    let mut store = Store::new(roots, 1 << 20, waker);
    assert!(
        store
            .scaled("movies", "poster.jpg", 12, 18, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    assert!(
...
fn empty_art_answers_none_without_a_decode() {
    ...
    let (wakes, waker) = wakes();
    let mut store = volume.store(1 << 20, waker);
    assert!(store.scaled("movies", "", 16, 16, Fit::Cover).is_none());
...
fn an_unknown_library_answers_none_without_a_decode() {
    ...
    assert!(
        store
            .scaled("shows", "poster.jpg", 16, 16, Fit::Cover)
            .is_none()
    );
    assert_eq!(settled_wakes(&wakes), 0);
}
...
fn a_zero_sized_box_answers_none_without_a_decode() {
    ...
    assert!(
        store
            .scaled("movies", "poster.jpg", 0, 16, Fit::Cover)
            .is_none()
    );
    assert!(
        store
            .scaled("movies", "poster.jpg", 16, 0, Fit::Cover)
            .is_none()
    );
    assert_eq!(settled_wakes(&wakes), 0);
}
...
fn a_contain_fit_keeps_the_ends_a_cover_fit_crops() {
    ...
    assert!(
        store
            .scaled("movies", "wide.png", 30, 30, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 1);
    let covered = store
    ...
    assert!(
        store
            .scaled("movies", "wide.png", 30, 30, Fit::Contain)
            .is_none()
    );
    wait_for_wakes(&wakes, 2);
    let fitted = store
...
fn a_pool_delivers_page_size_and_slot_size_decodes_together() {
    ...
    assert!(
        store
            .scaled("movies", "first.jpg", 1280, 800, Fit::Cover)
            .is_none()
    );
    assert!(
        store
            .scaled("movies", "second.jpg", 1280, 800, Fit::Cover)
            .is_none()
    );
    assert!(
        store
            .scaled("movies", "slot.jpg", 32, 32, Fit::Cover)
            .is_none()
    );
    wait_for_wakes(&wakes, 3);
    assert!(
