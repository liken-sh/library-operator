...
fn scaled(width: u32, height: u32, pixel: [u8; 4]) -> Scaled {
    Scaled::new(
        width,
        height,
...
fn encoded(key: &Key, scaled: &Scaled) -> Vec<u8> {
    encode(key, STAMP, scaled).expect("an encoded entry")
}

fn hit(bytes: &[u8], key: &Key) -> Scaled {
    let ReadOutcome::Hit(scaled) = parse(bytes, key, Some(STAMP)) else {
        panic!("a cache hit");
    };
    scaled
}
...
fn transparent_pixels_use_png_and_keep_their_exact_alpha() {
    ...
    let bytes = encoded(&key, &scaled(2, 1, [10, 20, 30, 117]));
...
fn opaque_pixels_use_quality_ninety_jpeg() {
    ...
    let bytes = encoded(&key, &scaled(16, 16, [30, 80, 140, 255]));
...
fn a_contain_entry_keeps_its_smaller_output_dimensions() {
    ...
    let bytes = encoded(&key, &scaled(60, 20, [10, 20, 30, 255]));
    let decoded = hit(&bytes, &key);
...
fn decoded_dimensions_must_match_the_header() {
    let key = key(60, 60, Fit::Contain);
    let mut bytes = encoded(&key, &scaled(60, 20, [10, 20, 30, 255]));
...
fn changed_source_metadata_is_a_miss() {
    ...
    let bytes = encoded(&key, &scaled(8, 8, [0, 0, 0, 255]));
...
fn a_changed_header_fails_its_digest_for_a_deleted_source() {
    let key = key(8, 8, Fit::Cover);
    let mut bytes = encoded(&key, &scaled(8, 8, [0, 0, 0, 255]));
    bytes[27] ^= 1;
