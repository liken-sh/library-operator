...
// The browser dims its whole frame from the second it asks for a film, so
// the film fades in over a dimmed page and the page under a
...
// is: the second the ask entered the state, and the second the lift
// began.
...
pub struct Lights {
    // The second the ask for a film entered the state.
    entered: f64,
...
impl Lights {
    /// The state the ask for a film enters at this second.
    ...
    /// keeps the lift it runs, so a second ask does not restart it.
...
mod tests {
    ...
    // The second the ask for a film lands on, which every case measures
    // from.
    const ASK: f64 = 3.0;
    ...
    #[test]
    fn the_room_is_at_full_when_the_ask_lands() {
        let lights = Lights::entered(ASK);
        assert_eq!(lights.level(ASK), 1.0);
        assert!(!lights.done(ASK));
    }
    ...
    fn the_room_goes_to_the_floor_over_the_way_down() {
        ...
        for (at, level) in [
            (ASK, 1.0),
            (ASK + look::LIGHTS_DOWN / 2.0, half()),
            (ASK + look::LIGHTS_DOWN, look::LIGHTS_FLOOR),
        ] {
    ...
    fn the_room_holds_the_floor_with_no_ceiling() {
        ...
        for held in [1.0, 60.0, 3_600.0] {
            let at = ASK + look::LIGHTS_DOWN + held;
    ...
    fn the_lift_runs_the_room_from_the_floor_back_to_full_and_ends() {
        let held = ASK + 10.0;
        let mut lights = Lights::entered(ASK);
        lights.lift(held);
    ...
    fn a_lift_part_way_down_runs_the_room_up_from_where_it_stood() {
        let part = ASK + look::LIGHTS_DOWN / 2.0;
        let mut lights = Lights::entered(ASK);
    ...
    #[test]
    fn a_second_ask_to_lift_does_not_restart_it() {
        let held = ASK + 10.0;
        let mut lights = Lights::entered(ASK);
        lights.lift(held);
