...
// nowhere and the watchdog stays off.
...
mod tests {
    ...
    fn the_grace_runs_from_the_launch() {
        let (watchdog, now) = armed(15);
        assert!(watchdog.counting());
        assert!(!watchdog.late(now + Duration::from_secs(14)));
        assert!(watchdog.late(now + Duration::from_secs(15)));
    }

    #[test]
    fn a_window_stops_the_grace() {
        let (mut watchdog, now) = armed(15);
        watchdog.present();
        assert!(!watchdog.counting());
    ...
    fn a_window_that_goes_away_starts_the_grace_again() {
        ...
        assert!(watchdog.counting());
        assert!(!watchdog.late(now + Duration::from_secs(74)));
        assert!(watchdog.late(now + Duration::from_secs(75)));
    }

    #[test]
    fn a_second_failure_does_not_extend_a_running_grace() {
        let (mut watchdog, now) = armed(15);
        watchdog.missing(now + Duration::from_secs(10));
        assert!(watchdog.late(now + Duration::from_secs(15)));
    }
    ...
    fn a_grace_that_has_not_run_out_leaves_the_process_alone() {
        let (watchdog, now) = armed(15);
        watchdog.expire_if_late(now + Duration::from_secs(14));
    }
