...
// scanner writes one alias per provider id it read from the .nfo file, and a
...
/// same seam every other local catalog read does. The numbers stay 0 here,
/// because the aliases table holds no episode number; the caller sets them.
