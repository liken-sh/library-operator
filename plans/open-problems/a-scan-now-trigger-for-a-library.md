...
`reenrich --only <fact>` validates against the 25-fact list in
`factnames.go`. The list is copied byte-for-byte into `cli/facts.go`,
because the CLI image builds from `cli/` alone and cannot import the
operator package. Nothing fails when the two lists drift. A fix moves
the vocabulary into a package both the operator and the CLI import, or
adds a test that reads both and reports a difference.
