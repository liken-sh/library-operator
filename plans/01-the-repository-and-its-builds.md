...
feature, built small so each later plan changes less. At the end of
...
images, a Rust browser image, and a Corrosion sidecar image. It also
names a release channel, a documentation site, and a local harness.
...
the skeleton is its own plan, and it delivers builds.

## The contract

The repository follows `media-operator`'s shape wherever the two share
a concern, so a person who reads one can read the other.
...
- A Go module at the root for the operator. It builds the way
  ...
  plan the operator starts, connects, and logs that it has no
  `Library` to reconcile.
...
- Images. One for the operator on `scratch`, as `media-operator`'s is.
  ...
  the browser on a distribution base, because Iced needs a Wayland
  and a Vulkan or EGL stack, as `media-operator`'s player image needs
  `mpv`'s. One for the Corrosion sidecar, built from a pinned
  Corrosion commit with the patches the catalog plan names.
- The release workflow, triggered by a tag, versioned by CalVer, and
  ...
  lock file, and the browser's Dockerfile builds dependencies in a
...
- `AGENTS.md`, with `CLAUDE.md` as a symlink to it, the brand theme as
  a submodule, and the documentation site scaffold. The reference
  pages generate from the CRD schemas, as the other operators' sites
  do.
- A `local/` directory in the shape of `media-operator/local/`: one
  script per thing a person can run on a workstation without a
  cluster. This plan adds the first, which runs the browser in a
  window with the harness flags. Later plans add a three-agent
  Corrosion cluster, a scanner against a local directory of films,
  and a browser against that catalog, so the whole read path runs on
  a laptop.
...
bases share nothing with the operator's `scratch`, and one file with
four stages is read four times as often as it is changed.

## Proof

CI is green on the first push: every image builds, both test suites
run, and a tag publishes a release with every artifact. On a
