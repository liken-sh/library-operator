...
responsibility from the design: renaming and moving files to a
library's naming convention.

## The problem

Radarr and Sonarr organize the lab's volume today. They rename and
...
those conventions from the `Library` settings blocks. A cluster that
replaces those tools, or that receives files another way, has nothing
that puts a file where the convention says it goes.

## The shape

- The organizer is a loop of its own, and the only loop that moves or
  renames anything. The scanner reads, the enricher adds sidecars
  beside files, and the organizer moves folders.
...
organizer also owns an inbox: a folder where new files land before
they have a place.
