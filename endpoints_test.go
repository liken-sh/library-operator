...
// ScannerPodAt is a scan Job's pod as the API server answers one: a
// name, a namespace, a UID, a node, and an address, with the member
// label that puts it in the namespace's catalog cluster.
func scannerPodAt(name, namespace, address, node string) Pod {
	return Pod{
		Metadata: ObjectMeta{
			...
			Namespace: namespace,
			UID:       name + "-uid",
			Labels:    withMemberLabel(workerLabels("movies", workerScan)),
		},
...
func screenPodAt(name, namespace, address, node string) Pod {
	...
	pod.Metadata.Labels = screenLabels(name)
	pod.Status.ContainerStatuses = []ContainerStatus{{Name: browserContainer, Ready: true}}
	return pod
}

// a screen's catalog agent gossips like a Job's, so a ready screen pod
...
// a pod with an address is a peer of its own namespace's cluster. A pod
...
// with no member pods the slice still exists, and it carries an
// empty endpoints list and not null.
func TestCatalogEndpointsWithNoMemberPodsAreEmptyAndNotNull(t *testing.T) {
