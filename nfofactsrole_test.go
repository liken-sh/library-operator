...
func TestAnNFOFactWritesItsGroupAndSaysWhoAnswered(t *testing.T) {
	for _, fact := range nfoFacts {
		t.Run(fact, func(t *testing.T) {
			...
			meta, err := parseMovieNFO([]byte(readFileString(t, sidecar)))
			if err != nil {
				t.Fatal(err)
			}
			...
			if len(ledger.Items) != 1 || ledger.Items[0].Wrote == "" {
				t.Fatalf("items = %+v, want one with the hash it wrote", ledger.Items)
			}
